...
export class SqlitePublishEventStore implements IPublishEventStore {
  ...
  create(event: PublishEvent): PublishEvent {
    this.db.prepare(`
      INSERT INTO publish_events (id, artifactId, publishedAt, destination, status, publishedBy,
  ...
  getById(id: string): PublishEvent | undefined {
    const row = this.db.prepare('SELECT * FROM publish_events WHERE id = ?').get(id) as Record<string, unknown> | undefined;
  ...
  listByArtifactId(artifactId: string): PublishEvent[] {
    const rows = this.db.prepare('SELECT * FROM publish_events WHERE artifactId = ?').all(artifactId) as Record<string, unknown>[];
    return rows.map((r) => this.rowToEvent(r));
  }

  listByStatus(status: PublishEventStatus): PublishEvent[] {
    const rows = this.db.prepare('SELECT * FROM publish_events WHERE status = ?').all(status) as Record<string, unknown>[];
    return rows.map((r) => this.rowToEvent(r));
  }
  ...
  updateStatus(
    ...
    const existing = this.db.prepare('SELECT * FROM publish_events WHERE id = ?').get(id) as Record<string, unknown> | undefined;
    ...
    if (updates) {
      if (updates.approvedBy !== undefined) { setClauses.push('approvedBy = ?'); params.push(updates.approvedBy); }
      if (updates.approvedAt !== undefined) { setClauses.push('approvedAt = ?'); params.push(updates.approvedAt); }
      if (updates.externalUrl !== undefined) { setClauses.push('externalUrl = ?'); params.push(updates.externalUrl); }
      if (updates.failureReason !== undefined) { setClauses.push('failureReason = ?'); params.push(updates.failureReason); }
      if (updates.retryCount !== undefined) { setClauses.push('retryCount = ?'); params.push(updates.retryCount); }
    }
