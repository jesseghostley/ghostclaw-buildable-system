...
import type { PlannerAction } from '../../../../planner/src/signal_router';
...
export class SqlitePlanStore implements IPlanStore {
  ...
  private rowToPlan(row: Record<string, unknown>): Plan {
    const plan: Plan = {
      ...
      action: row.action as PlannerAction,
    ...
    if (row.requiredAgents != null) plan.requiredAgents = JSON.parse(row.requiredAgents as string) as string[];
    if (row.expectedOutputs != null) plan.expectedOutputs = JSON.parse(row.expectedOutputs as string) as string[];
    return plan;
  }
  ...
  listBySignalId(signalId: string): Plan[] {
    const rows = this.db.prepare('SELECT * FROM plans WHERE signalId = ? ORDER BY createdAt ASC').all(signalId) as Record<string, unknown>[];
    return rows.map((r) => this.rowToPlan(r));
  }
